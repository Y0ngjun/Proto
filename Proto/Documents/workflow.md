...
*   **구조 파악 및 기획 (Architecture & Design)**
    *   `Documents/spec/architecture.md`: 엔진 4계층 아키텍처 및 데이터 흐름
    *   `Documents/spec/design.md`: 프로젝트 타겟, 요구사항, 주요 특징
*   **진행 상태 및 작업 목표 (Status & Tasks)**
    *   `Documents/progress.md`: 전체 로드맵 및 현재 구현 상태
*   **개발 규칙 (Rules & Guidelines)**
    *   `Documents/workflow.md`: 작업 시작 전 계획 수립(Phase A/B) 및 완료 기준 (현재 문서)
    *   `Documents/style-guide.md`: C++ 네이밍 컨벤션, 포맷팅, 주석 가이드
    *   `Documents/testing.md`: 테스트 작성(AAA 패턴) 및 실행 방법
    *   `Documents/refactoring.md`: 리팩토링 순서 및 사이클 지침
...
2. 참조 문서 확인: 작업 영역에 맞는 위 가이드의 문서를 확인. (예: Renderer 관련 작업 시 `src/Renderer.md` 필히 확인).
...
3. 검증: 빌드 및 테스트(테스트 가이드는 `Documents/testing.md` 참고) 실행.
4. 문서 업데이트: 변경된 내용이 있다면 관련 `.md` 갱신 (예: Core 코드 수정 시 `src/Core.md` 필히 업데이트).
...
## 완료 기준
- 빌드 성공 및 테스트 통과
- 진행 상황에 따라 `Documents/progress.md` 갱신
- 수정된 모듈에 해당하는 상세 구현 문서(`src/*.md`) 갱신
- 결과 요약 제공
