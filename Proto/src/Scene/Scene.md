...
- **Transform**: 객체의 위치(Translation), 회전(Rotation), 크기(Scale)를 관리하며 오일러-쿼터니언 변환으로 월드 행렬을 계산합니다. 부모-자식 계층은 `Scene::GetWorldTransform`이 루트까지 로컬 변환을 누적해 월드 행렬을 만들며, `Scene::SetParent`는 부모 변경 시 `inverse(newParentWorld)×oldWorld`로 로컬을 재계산하여 **화면상 월드 위치를 보존**합니다.
