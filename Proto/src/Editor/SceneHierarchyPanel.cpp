...
namespace Proto
{
	...
	void SceneHierarchyPanel::OnImGuiRender()
	{
		...
		{
			if (ImGui::MenuItem("Create Empty Object")) m_Context->CreateGameObject("Empty Object");
			if (ImGui::MenuItem("Create Cube")) m_Context->CreateMeshGameObject("Cube", UUID(1));
			if (ImGui::MenuItem("Create Sphere")) m_Context->CreateMeshGameObject("Sphere", UUID(3));
			if (ImGui::MenuItem("Create Cylinder")) m_Context->CreateMeshGameObject("Cylinder", UUID(4));
			if (ImGui::MenuItem("Create Plane")) m_Context->CreateMeshGameObject("Plane", UUID(2));
			ImGui::EndPopup();
		}
		...
		if (m_RenamingGameObject)
		{
			...
			if (ImGui::InputText("##RenameInput", m_RenamingBuffer, RenameBufferSize, ImGuiInputTextFlags_EnterReturnsTrue))
			{
	...
	void SceneHierarchyPanel::DrawEntityNode(GameObject* gameObject)
	{
		...
		if (ImGui::BeginPopupContextItem("EntityContextMenu"))
		{
			...
			{
				if (m_SelectionContext == gameObject) m_SelectionContext = nullptr;
				if (m_RenamingGameObject == gameObject) m_RenamingGameObject = nullptr;
